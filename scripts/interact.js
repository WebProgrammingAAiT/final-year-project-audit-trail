...
async function testReceiving() {
    ...
    const receive = await auditTrailContract.createReceivingTransaction("hash2", 'id', 'src', 'recno','user','ttype', recitems, newitems);
...
async function testTransfer() {
    ...
    const create = await auditTrailContract.createTransferringTransaction("hash", "id2", "reqtrans", "dept", "recno", "user", "transferringTransaction", transitems, newItems);
...
async function testReturn() {
    ...
    const create = await auditTrailContract.createReturningTransaction("hash2", 'id3', 'dept2', 'retdate2', 'recno2', 'user', 'type', resitems);
...
async function testRequest() {
    ...
    const create = await auditTrailContract.createRequestingTransaction("hash2", 'id4', 'dept', 'reqdate', 'recno', 'user', 'type', resitems);
    console.log(create.hash);
    trails.push({"id":"tid", "data":"dataHash", "hash":create.hash});
    trailsUpdate.emit('NewTrail');
    await create.wait();
